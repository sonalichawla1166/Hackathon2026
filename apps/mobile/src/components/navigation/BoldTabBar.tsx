...
import { colors, gradients, motion, radius, shadow } from '@/theme';
  ...
  return (
    <View style={styles.bar} onLayout={(e) => setContainerWidth(e.nativeEvent.layout.width)}>
      {containerWidth > 0 && (
        <Animated.View
          pointerEvents="none"
          style={[styles.pillWrap, { width: tabWidth, transform: [{ translateX: pillX }] }, shadow.glow(colors.accent)]}
        >
      ...
      {tabs.map(({ key, label, Icon }) => {
        ...
        return (
          <Pressable key={key} onPress={() => onPick(key)} style={styles.item}>
            <Icon size={active ? 23 : 20} color={active ? '#fff' : colors.textMuted} />
