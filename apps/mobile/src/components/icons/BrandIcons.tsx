import React from 'react';
import Svg, { Circle, Path } from 'react-native-svg';

interface IconProps {
  size?: number;
  color?: string;
}

/** Google's four-colour "G". Fixed brand colours by design — it must not be
 *  re-tinted for dark mode (Google's brand guidelines forbid it). */
export function GoogleIcon({ size = 18 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 48 48">
      <Path
        fill="#4285F4"
        d="M45.12 24.5c0-1.56-.14-3.06-.4-4.5H24v8.51h11.84c-.51 2.75-2.06 5.08-4.39 6.64v5.52h7.11c4.16-3.83 6.56-9.47 6.56-16.17z"
      />
      <Path
        fill="#34A853"
        d="M24 46c5.94 0 10.92-1.97 14.56-5.33l-7.11-5.52c-1.97 1.32-4.49 2.1-7.45 2.1-5.73 0-10.58-3.87-12.31-9.07H4.34v5.7C7.96 41.07 15.4 46 24 46z"
      />
      <Path
        fill="#FBBC05"
        d="M11.69 28.18c-.44-1.32-.69-2.73-.69-4.18s.25-2.86.69-4.18v-5.7H4.34A21.99 21.99 0 0 0 2 24c0 3.55.85 6.91 2.34 9.88l7.35-5.7z"
      />
      <Path
        fill="#EA4335"
        d="M24 10.75c3.23 0 6.13 1.11 8.41 3.29l6.31-6.31C34.91 4.18 29.93 2 24 2 15.4 2 7.96 6.93 4.34 14.12l7.35 5.7c1.73-5.2 6.58-9.07 12.31-9.07z"
      />
    </Svg>
  );
}

/** Filled sun — marks "switch to light mode". */
export function SunIcon({ size = 18, color = 'currentColor' }: IconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2} strokeLinecap="round">
      <Circle cx={12} cy={12} r={4.4} />
      <Path d="M12 2.4v2.4M12 19.2v2.4M2.4 12h2.4M19.2 12h2.4M5.2 5.2l1.7 1.7M17.1 17.1l1.7 1.7M18.8 5.2l-1.7 1.7M6.9 17.1l-1.7 1.7" />
    </Svg>
  );
}

/** Crescent moon — marks "switch to dark mode". */
export function MoonIcon({ size = 18, color = 'currentColor' }: IconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
      <Path d="M20.5 14.3A8.6 8.6 0 0 1 9.7 3.5a8.6 8.6 0 1 0 10.8 10.8z" />
    </Svg>
  );
}

/** Open / closed eye for the password reveal toggle. */
export function EyeIcon({ size = 18, color = 'currentColor', off = false }: IconProps & { off?: boolean }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round">
      <Path d="M1.8 12S5.5 5.3 12 5.3 22.2 12 22.2 12 18.5 18.7 12 18.7 1.8 12 1.8 12z" />
      <Circle cx={12} cy={12} r={3.1} />
      {off ? <Path d="M3.4 3.4l17.2 17.2" /> : null}
    </Svg>
  );
}

/** Door with an out-arrow — the sign-out action. */
export function LogoutIcon({ size = 18, color = 'currentColor' }: IconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round">
      <Path d="M9.4 20.6H5.6A1.6 1.6 0 0 1 4 19V5A1.6 1.6 0 0 1 5.6 3.4h3.8" />
      <Path d="M15.6 16.4 20 12l-4.4-4.4M20 12H9.4" />
    </Svg>
  );
}

/** Bell with a notification dot — the alerts button in the app header. */
export function BellIcon({ size = 18, color = 'currentColor' }: IconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round">
      <Path d="M18.2 16.4H5.8l1.3-2.2V10a4.9 4.9 0 0 1 9.8 0v4.2z" />
      <Path d="M10.2 19.2a2 2 0 0 0 3.6 0" />
    </Svg>
  );
}

/** Chevron pointing at a wall — collapses or expands the desktop sidebar.
 *  `open` draws it pointing left (collapse); otherwise right (expand). */
export function SidebarToggleIcon({ size = 18, color = 'currentColor', open = true }: IconProps & { open?: boolean }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round">
      <Path d={open ? 'M13.8 7.4 9.2 12l4.6 4.6' : 'M10.2 7.4 14.8 12l-4.6 4.6'} />
      <Path d="M18.4 4.6v14.8" />
    </Svg>
  );
}
